...
float CalculateAnimationInterpolant(in float elapsedTime, in float cycleDuration)
{
	...
	if (interpolant < 0.5f) {
		interpolant = 2.0 * interpolant;//0-1
	} else {
		interpolant = 1.0 - 2.0 * (interpolant - 0.5f);//1-0
	}
	return smoothstep(0.0, 1.0, interpolant);
}
...
inline Ray GenerateCameraRay(uint2 index, in float3 cameraPosition, in float4x4 projectionToWorld)
{
	...
	float2 screenPos = xy / DispatchRaysDimensions().xy * 2.0 - 1.0;//-1-1
...
{
	float costheta = clamp(abs((dot(normalize(I), normalize(N)))), 0, 1);
