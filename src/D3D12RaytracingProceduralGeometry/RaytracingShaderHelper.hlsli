...
float CalculateAnimationInterpolant(in float elapsedTime, in float cycleDuration)
{
	...
	if (interpolant <= 0.5f) {
		interpolant = 2.0f * interpolant;
	}
	else {
		interpolant = 2.0f - 2.0f * interpolant;
	}
...
inline Ray GenerateCameraRay(uint2 index, in float3 cameraPosition, in float4x4 projectionToWorld)
{
	...
	uint3 resolution = DispatchRaysDimensions();
	float4 screenSpace = { (2.0f * index.x / resolution.x) - 1.0f, 1.0f - (2.0f * index.y / resolution.y), 1.0f, 1.0f};
	float4 world = mul(normalize(screenSpace), projectionToWorld);
	ray.direction = normalize(float3(world.x, world.y, world.z));
...
{
	return f0 + (1 - f0)*pow(1 - saturate(dot(-I, N)), 5);;
}
