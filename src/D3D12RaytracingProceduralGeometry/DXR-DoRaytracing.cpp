...
void DXProceduralProject::DoRaytracing()
{
	...
	auto DispatchRays = [&](auto* raytracingCommandList, auto* stateObject, auto* dispatchDesc)
	{
		...
		{ m_hitGroupShaderTable->GetGPUVirtualAddress(), m_hitGroupShaderTable->GetDesc().Width ,m_hitGroupShaderTableStrideInBytes };

		// TODO-2.8: now fill in dispatchDesc->MissShaderTable
		dispatchDesc->MissShaderTable =
		{ m_missShaderTable->GetGPUVirtualAddress(), m_missShaderTable->GetDesc().Width , m_missShaderTableStrideInBytes };
