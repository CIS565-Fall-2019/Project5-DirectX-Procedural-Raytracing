...
void DXProceduralProject::InitializeScene()
{
	...
	// Setup materials.
	{
        ...
        // we can modify the data if we want to add more objects or colors in 
		auto SetAttributes = [&](
			UINT primitiveIndex,
...
{
    //currently we have 1 light and 2 AABBs in the scene?
...
// why camera stored in constant buffer? it is also a dynamic changed object
...
void DXProceduralProject::UpdateAABBPrimitiveAttributes(float animationTime)
{
	...
	auto SetTransformForAABB = [&](UINT primitiveIndex, XMMATRIX& mScale, XMMATRIX& mRotation)
	{
        ...
        m_aabbPrimitiveAttributeBuffer[primitiveIndex].localSpaceToBottomLevelAS = XMMatrixMultiply(XMMatrixMultiply(mScale, mRotation), mTranslation);
