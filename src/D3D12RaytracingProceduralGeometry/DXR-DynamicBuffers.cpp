...
void DXProceduralProject::UpdateAABBPrimitiveAttributes(float animationTime)
{
	...
	auto SetTransformForAABB = [&](UINT primitiveIndex, XMMATRIX& mScale, XMMATRIX& mRotation)
	{
        ...
        m_aabbPrimitiveAttributeBuffer[primitiveIndex].bottomLevelASToLocalSpace = mTransform;
        m_aabbPrimitiveAttributeBuffer[primitiveIndex].localSpaceToBottomLevelAS = XMMatrixInverse(nullptr, mTransform);
	};
