...
void DXProceduralProject::CreateHitGroupSubobjects(CD3D12_STATE_OBJECT_DESC* raytracingPipeline)
{
	...
	// the primitive types are Analytic and volumetric in raytracing scene defines
	{
		for (UINT rayType = 0; rayType < RayType::Count; rayType++)
		{
			for (UINT count = 0; count < IntersectionShaderType::Count; count++)
			{
				...
				//and intersection shaders (mandatory but not for triangles).
				// SetIntersectionShaderImport(LPCWSTR importName)
				// set the name, as of today this is either the analytic name or volumetric name
				hitGroup->SetIntersectionShaderImport(c_intersectionShaderNames[count]);

				// We tell the hitgroup that it should export into the correct shader hit group name, with the correct type
				hitGroup->SetHitGroupExport(c_hitGroupNames_AABBGeometry[count][rayType]);
...
void DXProceduralProject::CreateLocalRootSignatureSubobjects(CD3D12_STATE_OBJECT_DESC* raytracingPipeline)
{
	...
	// Very similar to triangles, except now one for each primitive type.
	{
		...
		auto rootSignatureAssociation = raytracingPipeline->CreateSubobject<CD3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION_SUBOBJECT>();
		rootSignatureAssociation->SetSubobjectToAssociate(*localRootSignature);
		// same as above but AABB has both these types
