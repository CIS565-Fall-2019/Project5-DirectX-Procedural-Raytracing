...
void DXProceduralProject::BuildGeometryDescsForBottomLevelAS(array<vector<D3D12_RAYTRACING_GEOMETRY_DESC>, BottomLevelASType::Count>& geometryDescs)
{
...

	{
		...
		for (UINT proceduralGeoIdx = 0; proceduralGeoIdx < IntersectionShaderType::TotalPrimitiveCount; proceduralGeoIdx++) {
			
			geometryDescs[BottomLevelASType::AABB][proceduralGeoIdx].AABBs.AABBs.StartAddress =
				m_aabbBuffer.resource->GetGPUVirtualAddress() +  proceduralGeoIdx*aabbDescTemplate.AABBs.AABBs.StrideInBytes;
		}
	}
}
...
AccelerationStructureBuffers DXProceduralProject::BuildTopLevelAS(AccelerationStructureBuffers bottomLevelAS[BottomLevelASType::Count], D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags)
{
	...
	//topLevelInputs.InstanceDescs = BottomLevelASType::Count; // only one instance per item 
	...
	AllocateUAVBuffer(device, topLevelPrebuildInfo.ScratchDataSizeInBytes, &scratch, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, L"ScratchForTLAS");
	...
	// Allocate space for the top-level AS.
	{
		...
		AllocateUAVBuffer(device, topLevelPrebuildInfo.ResultDataMaxSizeInBytes, &topLevelAS, initialResourceState, L"actualTLAS");
	}
...
void DXProceduralProject::BuildAccelerationStructures()
{
	...
	for (auto blasIdx = 0; blasIdx < BottomLevelASType::Count; blasIdx++)
		bottomLevelAS[blasIdx] = BuildBottomLevelAS(geometryDescs[blasIdx]);
	...
	for (auto blasCount = 0; blasCount < BottomLevelASType::Count; blasCount++) {
		m_bottomLevelAS[blasCount] = bottomLevelAS[blasCount].accelerationStructure;
	}
