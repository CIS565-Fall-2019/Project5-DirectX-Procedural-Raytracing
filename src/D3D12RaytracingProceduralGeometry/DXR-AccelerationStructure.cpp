...
void DXProceduralProject::BuildGeometryDescsForBottomLevelAS(array<vector<D3D12_RAYTRACING_GEOMETRY_DESC>, BottomLevelASType::Count>& geometryDescs)
{
...

	{
		...
		for (int aabbGeom = 0; aabbGeom < IntersectionShaderType::TotalPrimitiveCount; aabbGeom++) {
			// Geometry Desc variables
			geometryDescs[BottomLevelASType::AABB][aabbGeom].Type = D3D12_RAYTRACING_GEOMETRY_TYPE_PROCEDURAL_PRIMITIVE_AABBS;
			geometryDescs[BottomLevelASType::AABB][aabbGeom].Flags = geometryFlags;
			// Buffer and it's size
			geometryDescs[BottomLevelASType::AABB][aabbGeom].AABBs.AABBCount =
				m_aabbBuffer.resource->GetDesc().Width / sizeof(D3D12_RAYTRACING_AABB); // total size over data size, like above
...
AccelerationStructureBuffers DXProceduralProject::BuildBottomLevelAS(const vector<D3D12_RAYTRACING_GEOMETRY_DESC>& geometryDescs, 
