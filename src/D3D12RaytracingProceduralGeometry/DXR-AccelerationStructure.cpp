...
void DXProceduralProject::BuildGeometryDescsForBottomLevelAS(array<vector<D3D12_RAYTRACING_GEOMETRY_DESC>, BottomLevelASType::Count>& geometryDescs)
{
...

	{
		...
		for (UINT proceduralGeomCount = 0; proceduralGeomCount < IntersectionShaderType::TotalPrimitiveCount; proceduralGeomCount++)
		{
			geometryDescs[BottomLevelASType::AABB][proceduralGeomCount].AABBs.AABBs.StartAddress = m_aabbBuffer.resource->GetGPUVirtualAddress() + proceduralGeomCount * aabbDescTemplate.AABBs.AABBs.StrideInBytes;
		}
...
void DXProceduralProject::BuildAccelerationStructures()
{
	...
	for (auto geomCount = 0; geomCount < BottomLevelASType::Count ; geomCount++)
		bottomLevelAS[geomCount] = BuildBottomLevelAS(geometryDescs[geomCount]);
