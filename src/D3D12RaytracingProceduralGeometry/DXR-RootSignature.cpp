...
void DXProceduralProject::CreateRootSignatures()
{
	...
	// Think of them as local function parameters.
	{
		...
		//      --> the values they hold will depend on the shader function the local signature is bound to!
		{
            ...
            CD3DX12_ROOT_PARAMETER rootParameters[RootSignatureSlots::Count];
            rootParameters[RootSignatureSlots::MaterialConstant].InitAsConstantBufferView(SizeOfInUint32(PrimitiveConstantBuffer), 1);
            rootParameters[RootSignatureSlots::GeometryIndex].InitAsConstantBufferView(SizeOfInUint32(PrimitiveInstanceConstantBuffer), 1);
