...
void DXProceduralProject::CreateRootSignatures()
{
	...
	// Think of them as global function parameters.
	{
        ...
        ranges[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 1); //type of descriptor is SRV, 1 descriptor for indices, get resources from register 1
	...
	// Think of them as local function parameters.
	{
		...
		//      --> the values they hold will depend on the shader function the local signature is bound to!
		{
            ...
            rootParameters[RootSignatureSlots::MaterialConstant].InitAsConstants(SizeOfInUint32(PrimitiveConstantBuffer), 1);
            rootParameters[RootSignatureSlots::GeometryIndex].InitAsConstants(SizeOfInUint32(PrimitiveConstantBuffer), 2);; //registers 2 of the CBVs, but what is different between use InitAsconstants or bufferview
